...
export function WorkflowProvider({ children }: { children: ReactNode }) {
  ...
  const fetchWorkflows = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/workflows`,
        {
          headers: {
      ...
      if (response.status === 401) {
        ...
        // Do not clear here; AuthContext handles token lifecycle to avoid races
  ...
  const updateWorkflow = async (id: string, updates: Partial<Workflow>) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/workflows/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const deleteWorkflow = (id: string) => {
    ...
    addToast(`Delete "${workflow.name}"?`, "confirm", async () => {
      try {
        const token = getClientToken();
        if (!token) {
          addToast("Please login first", "error");

...
export function createDeployRouter(
  ...
  router.post('/', authMiddleware, async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `[Deploy] Found ${userCredentials.length} credentials for user`
      );

      // Check if user has all required credentials
      const credentialCheck = await checkRequiredCredentials(
        workflow,
        userCredentials,
        credentialRepo,
        userId
      ...
      console.log(
        "[Deploy] Workflow with credentials:",
        JSON.stringify(workflowWithCredentials, null, 2)
      );
      ...
      if (triggerNode) {
        if (triggerNode.type.includes("schedule")) {
          ...
          const cronExpr =
            triggerNode.parameters?.rule?.interval?.[0]?.expression;
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (executionMonitor && result.id) {
          console.log(
            `[Execute] 🔍 Starting execution monitor for ${result.id}`
          );
          executionMonitor
            .startMonitoring(result.id, workflowId, userId)
            .catch((err) => {
              console.error(
                "[Execute] Failed to start execution monitor:",
                err
              );
            });
        }
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `[Deactivate] User ${userId} deactivating workflow ${workflowId}`
        );
...
async function checkRequiredCredentials(
    ...
    if (serviceId) {
      ...
      if (!userServices.has(serviceId)) {
        ...
        if (integration) {
          ...
          const fields =
            integration.apiKey?.fields.map((field) => ({
              name: field.name,
              type:
                field.type === "password"
                  ? "password"
  ...
  const nodeList = missing
    .map((m) => `• ${m.nodeName} (requires ${m.service})`)
    .join("\n");
...
async function mapUserCredentialsToWorkflow(
  ...
  if (!userCredentials || userCredentials.length === 0) {
    console.log(
      "[Deploy] No user credentials found - workflow will need manual credential setup in n8n"
    );
    return workflow;
  }
    ...
    workflow.nodes.map(async (node) => {
      ...
      try {
        ...
        if (!credentialId) {
          ...
          await credentialRepo.updateN8nCredentialId(
            credential.id,
            credentialId
          );
          console.log(
            `[Deploy] Created n8n credential for ${service}: ${credentialId}`
          );
        } else {
          console.log(
            `[Deploy] Reusing existing n8n credential for ${service}: ${credentialId}`
          );
        }
